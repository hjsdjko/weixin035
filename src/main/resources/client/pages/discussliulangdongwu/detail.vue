...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussliulangdongwuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussliulangdongwuCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussliulangdongwu', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussliulangdongwu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussliulangdongwu');
			...
			async init(){
				...
				let res = await this.$api.info('discussliulangdongwu', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussliulangdongwu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussliulangdongwu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
