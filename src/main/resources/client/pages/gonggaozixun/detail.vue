...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussgonggaozixunCleanType')
			if(cleanType){
				uni.removeStorageSync('discussgonggaozixunCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('gonggaozixun', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','gonggaozixun');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('gonggaozixun',_this.detail)
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','gonggaozixun');
			...
			async init(){
				...
				let res = await this.$api.info('gonggaozixun', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussgonggaozixun', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussgonggaozixun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
