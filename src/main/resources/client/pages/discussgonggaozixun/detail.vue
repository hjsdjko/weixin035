...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussgonggaozixunCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussgonggaozixunCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussgonggaozixun', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussgonggaozixun');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussgonggaozixun');
			...
			async init(){
				...
				let res = await this.$api.info('discussgonggaozixun', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussgonggaozixun', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussgonggaozixun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
