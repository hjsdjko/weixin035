	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussgonggaozixun`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussgonggaozixun`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussgonggaozixun`, this.ruleForm);
						}else{
							await this.$api.add(`discussgonggaozixun`, this.ruleForm);
						}
						uni.setStorageSync('discussgonggaozixunCleanType',true);
